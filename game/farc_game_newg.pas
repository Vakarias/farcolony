...
///<summary>
///   commit the current setup of a new game and initialize the game interface
///</summary>
...
{:Purpose: commit the current setup of a new game and initialize the game interface.
   Additions:
      ...
      -2014Aug06- *code audit (BEGIN):
                  ...
                  (x)float local variables=> extended (x)case..of reformatting   (_)local methods: put inline; + reformat _X
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            {.spacecraft item}
            if FCDdgFactions[GNGselectedFactionIndex].F_colonizationModes[SelectedColonizationModeIndex].CM_equipmentList[Count0].EL_equipmentItem=feitSpaceUnit then
            begin
               {.init}
               ...
               if FCDdgFactions[GNGselectedFactionIndex].F_colonizationModes[SelectedColonizationModeIndex].CM_equipmentList[Count0].EL_eiSUnStatus=susInFreeSpace then
               begin
               ...
               begin
                  ...
                  if FCDdgFactions[GNGselectedFactionIndex].F_colonizationModes[SelectedColonizationModeIndex].CM_equipmentList[Count0].EL_eiSUnDockStatus=diMotherVessel then
                  begin
                     Count3:=Count2;
               ...
               else if FCDdgFactions[GNGselectedFactionIndex].F_colonizationModes[SelectedColonizationModeIndex].CM_equipmentList[Count0].EL_eiSUnStatus=susLanded then
               begin
               ...
               else if FCDdgFactions[GNGselectedFactionIndex].F_colonizationModes[SelectedColonizationModeIndex].CM_equipmentList[Count0].EL_eiSUnStatus=susDocked then
               begin
                  FCDdgEntities[Count1].E_spaceUnits[Count2].SU_status:=susDocked;
                  setlength( FCDdgEntities[Count1].E_spaceUnits[Count3].SU_dockedSpaceUnits, length( FCDdgEntities[Count1].E_spaceUnits[Count3].SU_dockedSpaceUnits ) + 1 );
                  FCDdgEntities[Count1].E_spaceUnits[Count3].SU_dockedSpaceUnits[length(FCDdgEntities[Count1].E_spaceUnits[Count3].SU_dockedSpaceUnits)-1].SUDL_index:=Count2;
      ...
      begin
         ...
         if FCDdgEntities[Count1].E_spmSettings[Count2].SPMS_isPolicy then
         begin
            ...
            if FCDdgEntities[Count1].E_spmSettings[Count2].SPMS_iPtIsSet then
            begin
               SPMItem:=FCFgSPM_SPMIData_Get( FCDdgEntities[Count1].E_spmSettings[Count2].SPMS_token );
               FCDdgEntities[Count1].E_spmMod_Cohesion:=FCDdgEntities[Count1].E_spmMod_Cohesion+SPMItem.SPMI_modCohes;
               FCDdgEntities[Count1].E_spmMod_Tension:=FCDdgEntities[Count1].E_spmMod_Tension+SPMItem.SPMI_modTens;
               FCDdgEntities[Count1].E_spmMod_Security:=FCDdgEntities[Count1].E_spmMod_Security+SPMItem.SPMI_modSec;
               FCDdgEntities[Count1].E_spmMod_Education:=FCDdgEntities[Count1].E_spmMod_Education+SPMItem.SPMI_modEdu;
               FCDdgEntities[Count1].E_spmMod_Natality:=FCDdgEntities[Count1].E_spmMod_Natality+SPMItem.SPMI_modNat;
               FCDdgEntities[Count1].E_spmMod_Health:=FCDdgEntities[Count1].E_spmMod_Health+SPMItem.SPMI_modHeal;
               FCDdgEntities[Count1].E_spmMod_Bureaucracy:=FCDdgEntities[Count1].E_spmMod_Bureaucracy+SPMItem.SPMI_modBur;
               FCDdgEntities[Count1].E_spmMod_Corruption:=FCDdgEntities[Count1].E_spmMod_Corruption+SPMItem.SPMI_modCorr;
               {:DEV NOTES: add SPMi custom effects application here.}
            end;
         end
         else if not FCDdgEntities[Count1].E_spmSettings[Count2].SPMS_isPolicy then
         begin
            ...
            FCMgSPMM_ModifiersCustomFx_Update(
               Count1
               ,Count2
               ,FCDdgEntities[Count1].E_spmSettings[Count2].SPMS_iPfSpreadValue
               ,true
               );
         end;
         inc(Count2);
   ...
   try
      ...
      FCMovM_3DView_Update( FCVdgPlayer.P_viewStarSystem, FCVdgPlayer.P_viewStar );
